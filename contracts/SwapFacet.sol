...
contract SwapFacet is VaultStorage, IFacet, Common {
            ...
            if (opType == 0) {
                ...
                for (uint256 j = 0; j < opTokenLength; j++) {
                    ...
                    _userBalances()[opDst][opTokens[j]] =
                        opTokens[j].toScaledBalance((opTokens[j].fromScaledBalance((_userBalances()[opDst][opTokens[j]])).toInt256() + opAmounts[j]).toUint256());
    ...
    function extort(
        ...
        for (uint256 j = 0; j < bribeTokens.length; j++) {
            ...
            if (g.totalVotes > 0) {
                ...
                unchecked {
                    _userBalances()[StorageSlot.getAddressSlot(SSLOT_HYPERCORE_TREASURY).value][token] += token.toScaledBalance(netDelta);
                }
            }
            ...
            if (index != type(uint256).max) {
                ...
                _userBalances()[user][token] += token.toScaledBalance(userClaimed);
            }
        }
        assembly ("memory-safe") {
    ...
    function _sendEmission(IGauge gauge) internal {
        ...
        if (_e().gauges[gauge].lastBribeUpdate == 1) {
            ...
            newEmissions = (
                uint256(_e().perVote - _e().gauges[gauge].perVoteAtLastEmissionUpdate) * _e().gauges[gauge].totalVotes
            ) / 1e9;
