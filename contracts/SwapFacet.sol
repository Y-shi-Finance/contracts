...
import {XYKPoolFactory, XYKPool} from "contracts/pools/xyk/XYKPoolFactory.sol";
...
contract SwapFacet is VaultStorage, IFacet, Common {
    ...
    constructor(IVC vc_, IWETH weth_, Token ballot_) {
        vc = vc_;
        ballot = ballot_;
        weth = weth_;
    ...
    function execute(
    ...
    ) external payable nonReentrant returns (int128[] memory) {
        uint256 tokenRefLength = tokenRef.length;
        require(
            tokenRefLength == deposit.length && tokenRefLength < 256,
            "malformed array"
        );
         ...
         */
        (bool orderChanged, , uint256[] memory toNewIdx) = _sort(
            tokenRef,
            deposit,
            ops
        );
            ...
            for (uint256 i = 0; i < tokenRefLength; ++i) {
                ...
                if (d > 0) {
                    deposit.u(
                        ...
                        int128(
                            uint128(
                                tokenRef.u(i).meteredTransferFrom(
                                    msg.sender,
                                    address(this),
                                    uint256(int256(d))
                                )
        ...
        if (msg.value > 0) {
            deposit.u(
                _binarySearchM(tokenRef, NATIVE_TOKEN),
        ...
        unchecked {
            for (uint256 i = 0; i < tokenRefLength; ++i) {
                ...
                if (d == 0) {
                    ...
                    tokenRef.u(i).transferFrom(
                        address(this),
                        msg.sender,
                        uint256(int256(d))
                    );
                } else {
                    tokenRef.u(i).safeTransferFrom(
                        msg.sender,
                        address(this),
                        uint256(int256(-d))
                    );
    ...
    function query(
        ...
        int128[] memory deposit,
        VelocoreOperation[] memory ops
    ) external nonReentrant returns (int128[] memory) {
        require(
            tokenRef.length == deposit.length && tokenRef.length < 256,
            "malformed input"
        );
        (bool orderChanged, , uint256[] memory toNewIdx) = _sort(
            tokenRef,
    ...
    function _execute(
        ...
        for (uint256 i = 0; i < ops.length; i++) {
                    ...
                    if (amountType == 0) {
                        ...
                        opAmounts.u(
                            ...
                            int128(
                                int256(
                                    Math.min(
                                        opTokens.u(j).balanceOf(address(this)),
            ...
            if (opType == 0) {
                // swap
                (int128[] memory deltaGauge, int128[] memory deltaPool) = ISwap(
                    opDst
                ).velocore__execute(user, opTokens, opAmounts, op.data);
                require(
                    deltaGauge.length == opTokenLength &&
                        deltaPool.length == opTokenLength
                );
                _verifyAndApplyDelta(
                    cumDelta,
                    IPool(opDst),
                    opTokens,
                    opTokenInformations,
                ...
                for (uint256 j = 0; j < opTokenLength; j++) {
                    ...
                    opTokens.u(j).transferFrom(
                        address(this),
                        opDst,
                        uint128(uint256(int256(opAmounts.u(j))))
                    );
                }

                IConverter(opDst).velocore__convert(
                    user,
                    opTokens,
                    opAmounts,
                    op.data
                ...
                int128[] memory deltas = SwapFacet(address(this)).balanceDelta(
                    opTokens,
                    balances
                );
                for (uint256 j = 0; j < opTokenLength; j++) {
                    ...
                    cumDelta[
                        uint8(uint256(opTokenInformations.u(j) >> (256 - 8)))
                    ] += deltas.u(j);
                ...
                if (gauge.lastBribeUpdate > 1) {
                    ...
                    if (elapsed > 0) {
                        ...
                        for (uint256 j = 0; j < len; j++) {
                            bytes memory extortCalldata = abi
                                .encodeWithSelector(
                                    SwapFacet.extort.selector,
                                    j,
                                    tokenRef,
                                    cumDelta,
                                    IGauge(opDst),
                            ...
                            assembly ("memory-safe") {
                                success := delegatecall(
                                    gas(),
                                    thisImpl,
                                    add(extortCalldata, 32),
                                    mload(extortCalldata),
                                    0,
                                    0
                                )
                                if success {
                ...
                if (ballotIndex != type(uint256).max) {
                    ...
                    if (deltaVote != type(int128).max) {
                        gauge.totalVotes = (int256(uint256(gauge.totalVotes)) +
                            deltaVote).toUint256().toUint112();
                        if (gauge.lastBribeUpdate != 1) {
                            _e().totalVotes = (int256(
                                uint256(_e().totalVotes)
                            ) + deltaVote).toUint256().toUint128();
                        }
                        gauge.userVotes[user] = (int256(
                            uint256(gauge.userVotes[user])
                        ) + deltaVote).toUint256().toUint128();
                ...
                for (uint256 j = 0; j < opTokenLength; j++) {
                    require(
                        isUser || opAmounts[j] >= 0,
                        "you can't withdraw other's balance"
                    );
                    _userBalances()[opDst][opTokens[j]] = opTokens[j]
                        .toScaledBalance(
                            (opTokens[j]
                                .fromScaledBalance(
                                    (_userBalances()[opDst][opTokens[j]])
                                )
                                .toInt256() + opAmounts[j]).toUint256()
                        );
                    cumDelta[
                        uint8(uint256(opTokenInformations.u(j) >> (256 - 8)))
                    ] -= opAmounts[j];
                ...
                unchecked {
                    for (uint256 j = 0; j < opTokenLength; j++) {
                        require(
                            int128(
                                uint128(uint256(opTokenInformations.u(j)))
                            ) >=
                                -cumDelta[
                                    uint8(
                                        uint256(
                                            opTokenInformations.u(j) >>
                                                (256 - 8)
                                        )
                                    )
                                ],
    ...
    ) external returns (int128[] memory delta) {
        delta = new int128[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            int128 diff = (tokens.u(i).balanceOf(address(this)).toInt256() -
                balancesBefore.u(i).toInt256()).toInt128();
            delta.u(i, diff);
            if (diff > 0) {
                tokens.u(i).transferFrom(
                    address(this),
                    address(0xDEADBEEF),
                    uint256(int256(diff))
                );
    ...
    function extort(
        ...
        if (address(briber) == address(gauge)) {
            // bypass bug
            if (
                address(briber) == 0xF4c92048F10b261e89f70c6DC4e4E9bBAC0f6ec3 ||
                XYKPoolFactory(0x75cB3eC310d3D1E22637F79D61eab5D9aBCD68BD)
                    .isPool(XYKPool(address(briber))) ||
                XYKPoolFactory(0xE1D6a7498DCBcA37DCB112018748C396bA749d66)
                    .isPool(XYKPool(address(briber)))
            ) {
                ...
                uint256 decay = 2 ** 32 -
                    rpow(XYKB(address(briber)).decayRate(), elapsed, 2 ** 32);
                uint256 minted = Math.ceilDiv(
                    (XYKB(address(briber)).totalSupply() + 1) *
                        XYKB(address(briber)).index().toUint256(),
                    1e18
                );
                (int256 a, int256 b, ) = XYKB(address(briber)).getReserves();
                uint256 actual = Math.sqrt(
                    (a.toUint256() + 1) * (b.toUint256() + 1)
                );
                uint256 excessInvariant = actual < minted ? 0 : actual - minted;
                uint256 decayed = (((excessInvariant * 1e18) /
                    XYKB(address(briber)).index().toUint256()) * decay) /
                    (2 ** 32);
                ...
                uint256 decay = 2 ** 32 -
                    rpow(XYKB(address(briber)).decayRate(), elapsed, 2 ** 32);
                uint256 minted = Math.ceilDiv(
                    (XYKB(address(briber)).totalSupply() + 2) *
                        XYKB(address(briber)).index().toUint256(),
                    1e18
                );
                uint256 actual = XYKB(address(briber)).invariant();
                uint256 excessInvariant = actual < minted ? 0 : actual - minted;
                uint256 decayed = (((excessInvariant * 1e18) /
                    XYKB(address(briber)).index().toUint256()) * decay) /
                    (2 ** 32);
                ...
                (bribeTokens, deltaGauge, deltaPool, deltaExternal) = briber
                    .velocore__bribe(gauge, elapsed);
            }
        } else {
            (bribeTokens, deltaGauge, deltaPool, deltaExternal) = briber
                .velocore__bribe(gauge, elapsed);
        ...
        require(
            bribeTokens.length == deltaGauge.length &&
                bribeTokens.length == deltaPool.length &&
                bribeTokens.length == deltaExternal.length
        );
        for (uint256 j = 0; j < bribeTokens.length; j++) {
            uint256 netDelta = (
                -(int256(deltaGauge.u(j)) + deltaPool.u(j) + deltaExternal.u(j))
            ).toUint256();
            ...
            if (g.totalVotes > 0) {
                ...
                unchecked {
                    _userBalances()[
                        StorageSlot
                            .getAddressSlot(SSLOT_HYPERCORE_TREASURY)
                            .value
                    ][token] += token.toScaledBalance(netDelta);
                }
            }

            uint256 userClaimed = ((r.current - r.snapshots[user]) *
                uint256(g.userVotes[user])) / 1e18;
            ...
            if (index != type(uint256).max) {
                ...
                _userBalances()[user][token] += token.toScaledBalance(
                    userClaimed
                );
            }
        }
    ...
    function _sort(
        ...
        returns (
            bool orderChanged,
            uint256[] memory toOldIdx,
            uint256[] memory toNewIdx
        )
    {
        ...
        unchecked {
            ...
            for (uint256 i = 1; i < tokenRefLength; ++i) {
                ...
                if (key <= tokens.u(i - 1)) {
                    ...
                    require(
                        tokenRefLength - 1 == j || tokens.u(j + 1) != key,
                        "duplicated token"
                    );
                    tokens.u(j, key);
                    toOldIdx.u(j, i);
            ...
            for (uint256 i = 0; i < opsLength; ++i) {
                ...
                for (uint256 j = 0; j < tokenInformationLength; ++j) {
                    ...
                    if (k >= 1 && key[0] <= arr.u(k - 1)[0]) {
                        ...
                        require(
                            arr.length - 1 == k || arr.u(k + 1)[0] != key[0],
                            "duplicated token in VelocoreOperation"
                        );
                        arr.u(k, key);
                    }
                }
                require(
                    tokenInformationLength == 0 ||
                        uint8(arr.u(tokenInformationLength - 1)[0]) <
                        tokenRefLength,
    ...
    function _dispenseVC() internal {
        if (_e().totalVotes > 0) {
            ...
            if (dispensed > 0) {
                _e().perVote =
                    _e().perVote +
                    ((uint256(1e9) * dispensed) / _e().totalVotes).toUint128();
            }
        }
    }
    ...
    function _sendEmission(IGauge gauge) internal {
        ...
        if (_e().gauges[gauge].lastBribeUpdate == 1) {
            ...
            newEmissions =
                (uint256(
                    _e().perVote -
                        _e().gauges[gauge].perVoteAtLastEmissionUpdate
                ) * _e().gauges[gauge].totalVotes) /
                1e9;
            ...
            _e().gauges[gauge].perVoteAtLastEmissionUpdate = uint112(
                _e().perVote
            );
        }
        _poolBalances()[gauge][toToken(vc)] = _poolBalances()[gauge][
            toToken(vc)
        ].credit(int256(newEmissions), 0);
    ...
    function _verifyAndApplyDelta(
        ...
        for (uint256 j = 0; j < opTokenLength; j++) {
            ...
            int128 d = dg + dp;
            require(
                d <= int128(uint128(uint256(tokenInformations.u(j)))),
                "token result above max"
            );
