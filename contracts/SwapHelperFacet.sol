...
contract SwapHelperFacet is VaultStorage, IFacet, Common {
    ...
    function initializeFacet() external {
        _setFunction(
            SwapHelperFacet(this).execute1.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).execute2.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).execute3.selector,
            thisImplementation
        );
        ...
        _setFunction(
            SwapHelperFacet(this).swapExactTokensForTokens.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).swapTokensForExactTokens.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).swapExactETHForTokens.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).swapTokensForExactETH.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).swapExactTokensForETH.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).swapETHForExactTokens.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).getAmountsOut.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).getAmountsIn.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).getPair.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).allPairs.selector,
            thisImplementation
        );
        _setFunction(
            SwapHelperFacet(this).allPairsLength.selector,
            thisImplementation
        );
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(path.length >= 1 && path.length < 256, "invalid path length");
        require(
            amountIn < uint256(int256(type(int128).max)) &&
                amountOutMin < uint256(int256(type(int128).max)) - 1,
            "invalid path length"
        );

        Token[] memory tokens = new Token[](path.length + 1);

        VelocoreOperation[] memory ops = new VelocoreOperation[](path.length);

        tokens[0] = _toToken(path[0].from);
        for (uint256 i = 0; i < path.length; i++) {
            require(i == 0 || path[i].from == path[i - 1].to);
            tokens[i + 1] = _toToken(path[i].to);
        }
        for (uint256 i = 0; i < ops.length; i++) {
            ...
            ops[i].tokenInformations[0] =
                bytes32(bytes1(uint8(i))) |
                bytes32(bytes2(uint16(2))) |
                bytes32(uint256(uint128(uint256(int256(type(int128).max)))));
            ops[i].tokenInformations[1] =
                bytes32(bytes1(uint8(i + 1))) |
                bytes32(bytes2(uint16(1)));
        }

        ops[ops.length - 1].tokenInformations[1] =
            bytes32(bytes1(uint8(ops.length))) |
            bytes32(bytes2(uint16(1))) |
        ...
        amounts[tokens.length - 1] = uint256(
            int256(execute(tokens, deposit, ops)[tokens.length - 1])
        );
        amounts[0] = amountIn;
    }
    ...
    function swapTokensForExactTokens(
        ...
        require(path.length >= 1 && path.length < 256, "invalid path length");
        require(
            amountInMax < uint256(int256(type(int128).max)) &&
                amountOut < uint256(int256(type(int128).max)) - 1,
            "invalid path length"
        );
        Token[] memory tokens = new Token[](path.length + 1);
        ...
        for (uint256 i = 0; i < ops.length; i++) {
            ops[i].poolId = bytes32(
                uint256(
                    uint160(
                        address(
                            _getPair(
                                tokens.u(ops.length - i - 1),
                                tokens.u(ops.length - i),
                                path[i].stable
                            )
                        )
                    )
            ...
            ops[i].tokenInformations[0] =
                bytes32(bytes1(uint8(ops.length - i - 1))) |
                bytes32(bytes2(uint16(1))) |
                bytes32(uint256(uint128(uint256(int256(type(int128).max)))));
            ops[i].tokenInformations[1] =
                bytes32(bytes1(uint8(ops.length - i))) |
                bytes32(bytes2(uint16(2)));
        }

        ops[ops.length - 1].tokenInformations[0] =
            bytes32(bytes1(uint8(0))) |
            bytes32(bytes2(uint16(1))) |
            bytes32(uint256(uint128(uint256(int256(amountInMax)))));
        ops[0].tokenInformations[1] =
            bytes32(bytes1(uint8(ops.length))) |
            bytes32(bytes2(uint16(0))) |
    ...
    function swapExactETHForTokens(
        ...
        require(path.length >= 1 && path.length < 256, "invalid path length");
        require(
            msg.value < uint256(int256(type(int128).max)) &&
                amountOutMin < uint256(int256(type(int128).max)) - 1,
            "invalid path length"
        );
        Token[] memory tokens = new Token[](path.length + 1);
    ...
    function getAmountsOut(
        ...
        amounts[tokens.length - 1] = uint256(
            int256(query(tokens, deposit, ops)[tokens.length - 1])
        );
        amounts[0] = amountIn;
    }
