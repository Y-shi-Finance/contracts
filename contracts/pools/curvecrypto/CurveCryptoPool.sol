...
contract CurveCryptoPool is ISwap, IBribe, SingleTokenGauge, PoolWithLPToken {
    ...
    function _exchange_for_lp(int256 r_base, int256 r_quote) internal returns (int256 delta_lp) {
        ...
        if (invariant != 0) {
            ...
            int256 fee_rate =
                (_fee(b_base + b_quote, (b_base * b_quote) / 1e18) * int256(invariantLastBlock)) / invariant;
    ...
    function _D(int256 s, int256 p, int256 d, int256 d_new) internal view returns (int256) {
        unchecked {
            ...
            int256 f_old;
            int256 f = (
                ((((((((1e18 * p) / d) * p) / d) * 1e18 + _coefficient_neg_2 * p) / d) * 1e18) + _coefficient_neg_1_e36)
                    / d
            ...
            for (uint256 i = 0; i < 255; i++) {
                ...
                f = (
                    (
                        (((((((1e18 * p) / d) * p) / d) * 1e18 + _coefficient_neg_2 * p) / d) * 1e18)
                            + _coefficient_neg_1_e36
                    ) / d
                ) + ((((_coefficient_2 * d) / 1e18) * d) / p) + _constant_term;
                if (f_old == f) {
                    return d;
    ...
    function cbrt(int256 k) internal pure returns (int256) {
        if (k >= 0) return int256(cbrt(uint256(k)));
        else return -int256(cbrt(uint256(-k)));
    }
